...
import {
  ...
  tap, scan, propagateEventTask, filter
} from '@most/core'
...
export interface Ray<A> extends rRay<A> {
  extend: <B extends A[K], K extends keyof A> (key: K, absurdB: Absurd<B>) =>
        (pith: Pith$<Ray<B>>) => void
  reduce: <K extends keyof A> (key: K, r: Stream<R<A[K]>>) => void
  onChange: Stream<A>
}

const sring = <A>(state$: Stream<A>) => ring<Ray<A>, rRay<A>>(pith => put => {
  pith({
    extend: <B extends A[K], K extends keyof A> (key: K, absurdB: Absurd<B>) =>
      (pith: Pith$<Ray<B>>) => {
