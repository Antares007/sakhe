...
export function updateAttributes(formerVNode: VNode<any>, vNode: VNode<any>) {
  let attributeValue: string | undefined
  let formerAttributeValue: string | undefined
  const element: Element = vNode.node
  let formerAttributes = formerVNode.data.attrs
  let attributes = vNode.data.attrs
  let attributeParts: Array<string> | undefined
