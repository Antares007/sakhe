...
const squareRing = pith => put => pith({
  square: (x, y, pith) => {
    const black = (x + y) % 2 === 1
    const style = {
      width: '12.5%',
...
const boardRing = pith => put => pith({
  board: (pith) => put.node(squareRing)('div.board', {
    style: {
      width: '90vw',
      height: '90vw',
...
const chessRing = pith => put => pith({
  chess: (key) => put.onode(apiRing, boardRing)(key, 'div.chess')((state, sselect) => {
    state.val(
      'kpos',
      sselect.action$
...
const id = a => a
const mapRaysRing = f => function map (pith) {
  return (...rays) => {
...
PatchBark(apiRing, p => mapRaysRing((put, ...rest) => [
  Object.assign({}, put, {
    node: pmap => (sel, data) => put.node(pmap)(sel + '.n', data),
    onode: (spmap, vpmap) => (key, sel, data) => put.onode(spmap, vpmap)(key, sel + '.o', data),
    anode: (spmap, vpmap) => (key, sel, data) => put.anode(spmap, vpmap)(key, sel + '.a', data)
  }),
  ...rest
])(chessRing(p)))(
  document.getElementById('root-node'),
  ...require('./initState')('chess')
)(
  Chess
).drain()
