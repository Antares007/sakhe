...
let fromPith mappend f =
    ...
    Pith.run
        ...
        <| f
    let line = s |> Map.toSeq |> Seq.sortBy fst |> Seq.toArray
    if Array.isEmpty line then None else Some << T <| line
let rec toPith = function
    | T a -> Pith.return' <| fun o -> Array.iter o a
    | LR (l, r) -> Pith.mappend Unit.mappend (toPith l) (toPith r)
...
let mappend mappend l r =
    ...
    let p = Pith.mappend Unit.mappend (toPith l) (toPith r)
    let o = fromPith mappend p
    o.Value
let rec runTo now tl = Pith.return' <| fun o ->
    match tl with
    | T a      ->
        if now < (fst a.[0]) then Some (T a)
        else
        a |> Seq.takeWhile (fun (a, _) -> a <= now) |> Seq.iter o
        if fst a.[a.Length - 1] <= now then None
        else
        Some (T (a |> Seq.skipWhile (fun (a, _) -> a <= now) |> Seq.toArray))
    | LR (l, r) ->
        match Pith.run o (runTo now l) with
        | None -> Pith.run o (runTo now r)
